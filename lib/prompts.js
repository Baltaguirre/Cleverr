  ...
  properties: {
    ...
    recommendations: {
      ...
      items: {
        ...
        required: ['name', 'note', 'query'],
        additionalProperties: false,
      },
    },
  },
  required: ['summary', 'digDeeper', 'recommendations'],
  additionalProperties: false,
})
...
Write a focused summary of the specific ideas, techniques or sections from THIS resource that matter most for the user's goal — concrete enough that they can start applying the core content without opening the source first. Be specific to ${subject} and their goal, and organize it as a few short points rather than an essay. Then write a short "digDeeper" pointer: what to actually go read or work through in the source itself if they want the full depth.

Finally, in "recommendations", suggest 3-4 real, well-known YouTube creators or channels worth following for this topic and the user's goal. Each is an object with "name" (the creator or channel), a one-line "note" on what they cover and why they help here, and "query" — a short YouTube search string that will surface their most relevant content. Recommend genuine creators you are confident exist; do NOT invent exact video URLs or video titles. Return JSON: { "summary": "...", "digDeeper": "...", "recommendations": [ { "name": "...", "note": "...", "query": "..." } ] }.`,
